...
ColorData ColorData::brighten(float delta_luminance) const {
    float add_red = delta_luminance * 0.2126;
    float add_green = delta_luminance * 0.7152;
    float add_blue = delta_luminance * 0.0722;

    return ColorData(this->red() + add_red, this->green() + add_green,
                     this->blue() + add_blue, this->alpha());
}
